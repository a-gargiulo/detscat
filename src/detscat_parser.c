...
struct DetScatParser {
    ...
    void *context;                  // Parser type-specific context
    Str current_line;               // Current line
    int line_number;                // Current line number
    DetScatParserStatus status;     // Parser status
    DetScatParserType type;         // Parser type
    bool eof;                       // End-of-file flag
    char error_message[256];        // Error message, 256 Bytes
};
...
static bool parse_double_cfg(const char *value, double *out, DetScatParser *parser,
                         const char *key) {
    assert(value != NULL && value[0] != '\0');
    assert(out != NULL);
    assert(parser != NULL);
...
static bool parse_int_cfg(const char* value, int *out, DetScatParser *parser, const char *key) {
    assert(parser != NULL);
...
static bool detscat_parser_parse_cfg_line(DetScatParser *parser, DetScatParserCfgContext *ctx) {
    assert(parser != NULL);
    assert(ctx != NULL);
    ...
    *equals = '\0';
    ...
    if (strcmp(key, "is_polarized") == 0) {
        ...
        if (!detscat_str_set(&cfg->particles_file_path, value)) {
            PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_FORMAT,
                     "Could not set '%s' at line %d", key, parser->line_number);
            return false;
        }
    } else if (strcmp(key, "camera_center_position_m") == 0) {
        int matched =
            sscanf(value, "[ %lf , %lf , %lf ]", &cfg->camera_center_position_m.x,
        ...
        PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_UNKNOWN_KEY,
                 "Unknown key '%s' at line %d", key, parser->line_number);
        return false;
    }
...
static bool detscat_prt_parse_type(DetScatParser *parser, DetScatPrtType *type,
    ...
    if (!line || !*line) {
        PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_FORMAT,
                 "Invalid particle type definition at line %d", parser->line_number);
        return false;
    }
    ...
    if (!detscat_str_set(&type->type_id, token1) || !detscat_str_set(&type->data_dir, token2)) {
        PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_MEMORY,
...
static bool detscat_prt_parse_particle(DetScatParser *parser,
    ...
    if (!tok) {
        free(copy);
        PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_FORMAT,
                 "Invalid particle definition at line %d", parser->line_number);
        return false;
    }
    if (strlen(tok) >= DETSCAT_PRT_TYPEID_MAX) {
        free(copy);
        PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_FORMAT,
                 "Particle type too long at line %d", parser->line_number);
        return false;
    }
    ...
    if (!detscat_str_set(&particle->type_id, type_id)) {
        PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_MEMORY,
                 "Could not set particle type ID at line %d", parser->line_number);
        goto error_cleanup;
    }
...
static bool detscat_parser_parse_prt_line(DetScatParser *parser, DetScatParserPrtContext *ctx) {
    assert(parser != NULL);
    assert(ctx != NULL);
    ...
    switch (ctx->state) {
        ...
        case STATE_PARSE_PARTICLES_DEF:
            ...
            if (strcmp(trimmed, "$(EndParticles)") == 0) {
                if (ctx->particles_allocated != prt->n_particles) {
                    PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_FORMAT,
                             ...
                             prt->n_particles, ctx->particles_allocated);
                    ctx->state = STATE_ERROR;
                    goto error_cleanup;
                }
            ...
            if (!detscat_str_init(&prt->particles[ctx->particles_allocated].type_id) ||
                !detscat_str_reserve(&prt->particles[ctx->particles_allocated].type_id,
                             DETSCAT_PRT_TYPEID_INIT)) {
                PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_MEMORY,
                         "Memory allocation failed for particle type ID "
                         "at line %d",
            ...
            prt->particles[ctx->particles_allocated].case_id = 
                calloc(1, sizeof(
                            *prt->particles[ctx->particles_allocated].case_id));
...
bool detscat_parser_parse_line(DetScatParser *parser) {
    ...
    switch (parser->type) {
        ...
        default:
            // unknown parser type — treat as success or error as appropriate
            return true;
    }
}
...
bool detscat_parser_init(DetScatParser *parser, const char *input_source, void *context) {
    ...
    errno = 0;
    parser->stream = fopen(input_source, "r");
    if (!parser->stream) {
        PARSER_SET_ERROR(parser, DETSCAT_PARSER_ERR_FILE,
                         "Could not open '%s': %s", input_source, strerror(errno));
        return false;
    }
...
bool detscat_parser_reset(DetScatParser *parser, const char *file_path) {
    ...
    if (!file_path || !*file_path) {
        parser->status = DETSCAT_PARSER_ERR_ARGUMENT;
    ...
    if (!detscat_str_init(&parser->current_line) ||
        !detscat_str_reserve(&parser->current_line, DETSCAT_PARSER_LINE_INIT)) {
        parser->status = DETSCAT_PARSER_ERR_MEMORY;
        snprintf(parser->error_message, sizeof(parser->error_message),
    ...
    switch (parser->type) {
        case DETSCAT_CFG:
             // if(!detscat_cfg_context_reset(&parser->context)) {
             //     detscat_cfg_context_free(&parser->context);
             //     parser->context = NULL;
             //     return false;    
             // };
             break;
        case DETSCAT_PRT:
             // if(!detscat_prt_context_reset(&parser->context)) {
             //     detscat_prt_context_free(&parser->context);
             //     parser->context = NULL;
             //     return false;    
             // };
             break;
    ...
    if (!parser->stream) {
        parser->status = DETSCAT_PARSER_ERR_FILE;
        snprintf(parser->error_message, sizeof(parser->error_message),
...
bool detscat_parser_set_error_message(DetScatParser *parser, const char *error_message) {
    ...
    snprintf(parser->error_message, sizeof(parser->error_message), "%s", error_message);
